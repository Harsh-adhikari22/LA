...
export function EventGrid({ searchQuery, categories, limit, showTrendingOnly, filters, sort }: EventGridProps) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12 rajdhani">
        ...
        <span className="ml-2 text-[#f2d47a]">Loading events...</span>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="text-center py-12 rajdhani">
        ...
        <button onClick={() => window.location.reload()} className="text-[#d4af37] hover:underline">
          Try again
        </button>
      </div>
    )
  ...
  if (events.length === 0) {
    return (
      <div className="text-center py-12 rajdhani">
        <p className="text-[#f2d47a] text-lg mb-2">No events found</p>
        <p className="text-[#c9a949]">Try adjusting your search criteria or check back later for new events.</p>
      </div>
    )
  }

  return (
    <div className="rounded-2xl border border-[#b88a22]/40 bg-black/50 backdrop-blur-md p-4 md:p-6 shadow-[0_0_30px_rgba(212,175,55,0.2)]">
